...
def map_settings(request):
    ...
    if request.method == 'POST':
        ...
        if 'new_setting' in request.POST:
            ...
            settings = request.POST.get('setting_id', None)
            print("settings" , settings)
            color = request.POST.get('color', 'R')
            delete_setting = request.POST.get('delete_setting', 0)
            map_settings = request.POST.get('map_settings', None)
            for i in range(0, len(settings)):
                setting = settings[i]
                if delete_setting[i] == 1:
                    Settings.objects.delete(id=setting.id)
                else:

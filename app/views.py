...
from .helper_functions import give_first_planet
from .tables import EmpireRankTable, UserRankTable
...
from app.map_settings import players_per_empire
...
def custom_login(request):
    ...
    if user is not None:
        ...
        context = {
            "news_feed": NewsFeed.objects.all().order_by("-date_and_time"),
...
def choose_empire_race(request):
    ...
    if request.POST and "chose_race" in request.POST and "chose_emp" in request.POST:
        if request.POST["chose_emp"] == "Random":
            ...
            if empire1.password or not (
                "fampass" in request.POST
                and empire1.password == request.POST["fampass"]
            ):
...
def council(request):
    ...
    for construction in constructions:
        label = (
            construction.get_building_type_display()
        )  # get_X_display() is the trick for getting the full label of the textchoice
...
def map(request):
    if request.GET.get("heatmap", None):
        ...
        for x in range(100):
            for y in range(100):
                for portal in portals:
                    ...
                    research_culture = 0
                    cover = np.max(
                        (0, 1.0 - np.sqrt(d / (7.0 * (1.0 + 0.01 * research_culture))))
                    )  # from battlePortalCalc()
                    ...
                    heatmap[
                        y, x
                    ] += cover  # FIXME no idea why but i had to swap x and y for it to match planets...
        ...
        show_heatmap = False
    status = get_object_or_404(UserStatus, user=request.user)
    systems = Planet.objects.filter(i=0).values_list(
        "x", "y"
...
def planets(request):
    ...
    if order_by == "planet":
        ...
        planets = Planet.objects.filter(owner=request.user).order_by(
            order_by
        )  # directly use the keyword

    context = {"status": status, "planets": planets, "page_title": "Planets"}
...
def planet(request, planet_id):
    ...
    if (
        planet.owner
    ):  # if planet is owned by someone, grab that owner's status, in order to get faction and other info of owner
...
def raze(request, planet_id):
    ...
    if request.method == "POST":
        ...
        for building in building_list:
            num_to_raze = request.POST.get(
                building.short_label, None
            )  # number user entered to raze for this building type
            if num_to_raze == "on":
                num_to_raze = 1
            elif num_to_raze:
                ...
                num_to_raze = 0
            num_on_planet = getattr(
                planet, building.model_name
            )  # This is how to access a field of a model using a string for the name of that field
            if (num_to_raze > 0) and (num_on_planet >= num_to_raze):
                setattr(planet, building.model_name, num_on_planet - num_to_raze)
                setattr(
                    planet,
                    "total_buildings",
                    getattr(planet, "total_buildings") - num_to_raze,
                )
                setattr(
                    status,
                    "total_" + building.model_name,
                    getattr(status, "total_" + building.model_name) - num_to_raze,
                )
                setattr(
                    status,
                    "total_buildings",
                    getattr(status, "total_buildings") - num_to_raze,
                )
                top_msg += (
                    "You razed " + str(num_to_raze) + " " + building.label + "<p><p>"
                )
            elif num_to_raze > 0:
                top_msg += (
                    "Did not have "
        ...
        planet.overbuilt = calc_overbuild(
            planet.size, planet.total_buildings + planet.buildings_under_construction
        )
    ...
    context = {
        ...
        "planet": planet,
        "top_msg": top_msg,
        "page_title": "Raze Buildings on Planet "
        + str(planet.x)
        + ","
        + str(planet.y)
        + ":"
        ...
        for building in building_list:
            ...
            if num_on_planet:
                ...
                setattr(
                    status,
                    "total_buildings",
                    getattr(status, "total_buildings") - num_on_planet,
                )
...
def build(request, planet_id):
    ...
    if request.method == "POST":
        ...
        for building in building_list:
            ...
            if num:
                ...
                total_resource_cost, penalty = building.calc_cost(
                    num,
                    status.research_percent_construction,
                    status.research_percent_tech,
                )
                if not total_resource_cost:
                    msg += (
                        "Not enough tech research to build " + building.label + "<br>"
                    )
                    continue

                total_resource_cost = ResourceSet(
                    total_resource_cost
                )  # convert to more usable object
                ob_factor = calc_overbuild_multi(
                    planet.size,
                ...
                total_resource_cost.apply_overbuild(
                    ob_factor
                )  # can't just use planet.overbuilt, need to take into account how many buildings we are making
    ...
    for building in building_list:
        ...
        cost_list, penalty = building.calc_cost(
            1, status.research_percent_construction, status.research_percent_tech
        )
        ...
        if cost_list:  # Remember that cost_list will be None if the tech is too low
            ...
            for i in range(5):  # 4 types of resources plus time
                cost_list_labeled.append(
                    {
                        "value": int(np.ceil(cost_list[i] * planet.overbuilt)),
...
def ranking(request):
    status = get_object_or_404(UserStatus, user=request.user)
    table = UserRankTable(
        UserStatus.objects.exclude(race__isnull=True)
        .exclude(race__exact="")
        .exclude(empire__isnull=True),
        order_by=("-num_planets"),
    )
    context = {"table": table, "status": status}
...
def empire_ranking(request):
    ...
    table = EmpireRankTable(
        Empire.objects.all().filter(numplayers__gt=0), order_by=("-planets")
    )
    context = {"table": table, "status": status, "empire": empire}
...
def units(request):
    ...
    if request.method == "POST":
        ...
        for i, unit in enumerate(unit_info["unit_list"]):
            ...
            num = request.POST.get(
                str(i), 0
            )  # must match name of each inputfield in the template, in this case we are using integers
            if num == "":
                num = 0
            else:
                num = int(num)
            if num:
                ...
                if not mult:
                    msg += (
                        "Not enough tech research to build "
                ...
                total_resource_cost = [
                    int(np.ceil(x * mult)) for x in unit_info[unit]["cost"]
                ]
                for j in range(
                    4
                ):  # multiply all resources except time by number of units
                ...
                UnitConstruction.objects.create(
                    user=request.user,
                    n=num,
                    unit_type=unit,
    ...
    unit_dict = (
        []
    )  # idea here is to build up a dict we can nicely iterate over in the template
    for unit in unit_info["unit_list"]:
        if unit == "phantom":
            continue
        d = {}
        mult, penalty = unit_cost_multiplier(
            status.research_percent_construction,
            status.research_percent_tech,
            unit_info[unit]["required_tech"],
        )
        if not mult:
            ...
            for i, resource in enumerate(resource_names):
                cost.append(
                    {
                        "name": resource,
                        "value": int(np.ceil(mult * unit_info[unit]["cost"][i])),
                    }
                )
            d["cost"] = cost
        d["penalty"] = penalty
        ...
        unit_dict.append(d)
    context = {"status": status, "unit_dict": unit_dict, "msg": msg}
...
def fleets(request):
    ...
    send_fleet_list = (
        []
    )  # need to have a separate list that doesnt include agents/psycics/ghosts/explos
    for unit in unit_info["unit_list"]:
        num = getattr(main_fleet, unit)
        if num:
            main_fleet_list.append(
                {
                    "name": unit_info[unit]["label"],
                    "value": num,
                    "i": unit_info[unit]["i"],
                }
            )
            if unit not in ["wizard", "agent", "ghost", "exploration"]:
                send_fleet_list.append(
                    {
                        "name": unit_info[unit]["label"],
                        "value": num,
                        "i": unit_info[unit]["i"],
...
def fleetsend(request):
    ...
    send_unit_dict = (
        {}
    )  # contains how many of each unit to send, dict so its quick to look up different unit counts
    total_sent_units = 0
    ...
    if (
        order == 0 or order == 1
    ):  # if attack planet or station on planet, make sure planet exists and get planet object
        try:
        ...
        if (
            x < 0
            or x >= round_params.galaxy_size
            or y < 0
            or y >= round_params.galaxy_size
        ):
    ...
    portal_planets = Planet.objects.filter(
        owner=request.user, portal=True
    )  # should always have at least the home planet
    min_dist = 9999999999
    ...
    speed = race_info_list[status.get_race_display()][
        "travel_speed"
    ...
    if send_unit_dict["carrier"] * 100 < (
        send_unit_dict["bomber"]
        + send_unit_dict["fighter"]
        + send_unit_dict["transport"]
    ):
        return HttpResponse(
    ...
    if send_unit_dict["transport"] * 100 < (
        send_unit_dict["soldier"]
        + send_unit_dict["droid"]
        + 4 * send_unit_dict["goliath"]
    ):
        return HttpResponse(
    ...
    Fleet.objects.create(
        owner=request.user, command_order=order, x=x, y=y, ticks_remaining=fleet_time
    )
    ...
    return HttpResponse(
        "The fleet will reach its destination in " + str(fleet_time) + " weeks<br>"
    )
...
def vote(request):
    ...
    try:
        ...
        context = {"status": status, "page_title": "Vote", "player_list": player_list}
...
def results(request):
    status = get_object_or_404(UserStatus, user=request.user)
    player_list = UserStatus.objects.filter(empire=status.empire)
    context = {"status": status, "page_title": "Results", "player_list": player_list}
...
def set_relation(relation, current_empire, target_empire_nr, *rel_time_passed):
    ...
    if relation == "nap":
        if (
            rel is not None
            and rel.relation_type == "NO"
            and rel.relation_length == rel_time
        ):
...
def cancel_relation(relation):
    ...
    if rel.relation_type == "AO" or rel.relation_type == "NO":
        ...
        if rel.relation_type == "A":
            ...
            rel2.save()
        if (
            RoundStatus.objects.get().tick_number - rel.relation_creation_tick
            > min_relation_time
        ):
            rel.delete()
...
def pm_options(request):
    ...
    if request.method == "POST":
        ...
        user_empire.name_with_id = (
            request.POST["empire_name"] + " #" + str(user_empire.number)
        )
        ...
        if request.POST["empire_offer_nap"]:
            set_relation(
                "nap",
                status.empire,
                request.POST["empire_offer_nap"],
                request.POST["empire_offer_nap_hours"],
            )
...
def research(request):
    ...
    if request.method == "POST":
        ...
        if "rc_alloc_form" in request.POST:
            ...
            if total != 100:
                message = (
                    "Research allocation percentages must be equal to 100% in total!"
                )
            else:
    ...
    context = {"status": status, "page_title": "Research", "message": message}
...
def famgetaid(request):
    ...
    if "receive_aid" in request.POST:
        status2 = get_object_or_404(UserStatus, user=request.POST["player"])
        if (
            status2.request_aid == "A"
            or (status2.request_aid == "PM" and status.empire_role == "PM")
            or (
                status2.request_aid == "VM"
                and (status.empire_role == "PM" or status.empire_role == "VM")
            )
        ):
            if request.POST["energy"]:
                e = int(request.POST["energy"])
            ...
            if request.POST["ectrolium"]:
                ...
                if e > status2.ectrolium:
                    message += (
                        status2.user_name + " doesn't have so much ectrolium!<br>"
                    )
                else:
                    status.ectrolium += e
                    status2.ectrolium -= e
...
def messages_(request):
    status = get_object_or_404(UserStatus, user=request.user)
    messages_from = Messages.objects.filter(
        user2=status.id, user2_deleted=False
...
def outbox(request):
    status = get_object_or_404(UserStatus, user=request.user)
    messages_to = Messages.objects.filter(
        user1=status.id, user1_deleted=False
    ).order_by("-date_and_time")
    context = {
        "status": status,
        "page_title": "Outbox",

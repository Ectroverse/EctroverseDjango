...
from app.calculations import (
    battlePortalCalc,
    calc_overbuild,
    specopEnlightemntCalc,
    specopSolarCalc,
)
from app.constants import (
    population_size_factor,
    stockpile,
    energy_decay_factor,
    race_info_list,
    building_production_solar,
    building_production_fission,
    building_production_mineral,
    building_production_ectrolium,
    building_production_crystal,
    building_production_research,
    building_production_cities,
    upkeep_solar_collectors,
    upkeep_fission_reactors,
    upkeep_mineral_plants,
    upkeep_refinement_stations,
    upkeep_crystal_labs,
    upkeep_research_centers,
    upkeep_cities,
    upkeep_defense_sats,
    upkeep_shield_networks,
    networth_per_building,
    settings_num_value,
    crystal_decay_factor,
)
...
class Command(
    BaseCommand
):  # must be called command, use file name to name the functionality
    ...
    def handle(self, *args, **options):
        ...
        # update relations
        relations_buffer = Relations.objects.filter(
            Q(relation_type="W") | Q(relation_type="NC")
        )
        ...
        Relations.objects.filter(
            Q(relation_type="W") | Q(relation_type="NC"), relation_remaining_time__lte=0
        ).delete()
        ...
        for status in UserStatus.objects.all():
            ...
            */
            ...
            # Repeat for units
            main_fleet = Fleet.objects.get(
                owner=status.user.id, main_fleet=True
            ...
            for job in UnitConstruction.objects.filter(user=status.user.id):
                ...
                if job.ticks_remaining <= 0:
                    setattr(
                        main_fleet,
                        job.unit_type,
                        getattr(main_fleet, job.unit_type) + job.n,
                    )
                    main_fleet.save()
                    # TODO ADD IT TO THE NEWS!
                    job.delete()
            ...
            for planet in planets_buffer:
                ...
                planet.max_population *= (
                    1.00 + 0.01 * status.research_percent_population
                )
                ...
                # else
                nInfection = 0
                planet.current_population += int(
                    np.ceil(
                        planet.current_population
                        * race_info["pop_growth"]
                        * (1.00 + 0.01 * status.research_percent_population)
                        * 0.75 ** nInfection
                    )
                )
                planet.current_population = min(
                    planet.max_population, planet.current_population
                )

                if planet.portal:
                    planet.protection = 100
                else:
                    planet.protection = int(
                        100.0
                        * battlePortalCalc(
                            planet.x,
                            planet.y,
                            portal_xy_list,
                            status.research_percent_culture,
                        )
                    )
                ...
                planet.total_buildings = (
                    planet.solar_collectors
                    + planet.fission_reactors
                    + planet.mineral_plants
                    + planet.crystal_labs
                    + planet.refinement_stations
                    + planet.cities
                    + planet.research_centers
                    + planet.defense_sats
                    + planet.shield_networks
                    + int(planet.portal)
                )
                ...
                # 	factor = 2.0
                cmdTickProduction_solar += (
                    building_production_solar * planet.solar_collectors
                ) * (1 + factor * planet.bonus_solar / 100.0)
                cmdTickProduction_fission += (
                    building_production_fission * planet.fission_reactors
                )
                cmdTickProduction_mineral += (
                    building_production_mineral * planet.mineral_plants
                ) * (1 + factor * planet.bonus_mineral / 100.0)
                cmdTickProduction_crystal += (
                    building_production_crystal * planet.crystal_labs
                ) * (1 + factor * planet.bonus_crystal / 100.0)
                cmdTickProduction_ectrolium += (
                    building_production_ectrolium * planet.refinement_stations
                ) * (1 + factor * planet.bonus_ectrolium / 100.0)
                cmdTickProduction_cities += building_production_cities * planet.cities
                cmdTickProduction_research += (
                    building_production_research * planet.research_centers
                )
                ...
                planet.overbuilt = calc_overbuild(
                    planet.size,
                    planet.total_buildings + planet.buildings_under_construction,
                )
                planet.overbuilt_percent = (
                    planet.overbuilt - 1.0
                ) * 100  # came from html_gameplay.c line 5541
            ...
            status.total_buildings = (
                status.total_solar_collectors
                + status.total_fission_reactors
                + status.total_mineral_plants
                + status.total_crystal_labs
                + status.total_refinement_stations
                + status.total_cities
                + status.total_research_centers
                + status.total_defense_sats
                + status.total_shield_networks
                + status.total_portals
            )
            ...
            status.research_points_military += race_info[
                "research_bonus_military"
            ] * status.alloc_research_military * (
                100 * cmdTickProduction_research
                + 1.2 * status.current_research_funding
                + artiBonus
            ) / 10000.0 * specopEnlightemntCalc(
                status.user.id, CMD_ENLIGHT_RESEARCH
            ) + int(
                race_info["race_special"] == "RACE_SPECIAL_POPRESEARCH"
            ) * 1.2 * (
                status.alloc_research_military * status.population
            ) / (
                600.0 * 100.0
            )
            status.research_points_construction += race_info[
                "research_bonus_construction"
            ] * status.alloc_research_construction * (
                100 * cmdTickProduction_research
                + 1.2 * status.current_research_funding
                + artiBonus
            ) / 10000.0 * specopEnlightemntCalc(
                ...
                status.alloc_research_construction * status.population
            ) / (
                600.0 * 100.0
            )
            status.research_points_tech += race_info[
                "research_bonus_tech"
            ] * status.alloc_research_tech * (
                100 * cmdTickProduction_research
                + 1.2 * status.current_research_funding
                + artiBonus
            ) / 10000.0 * specopEnlightemntCalc(
                ...
                status.alloc_research_tech * status.population
            ) / (
                600.0 * 100.0
            )
            status.research_points_energy += race_info[
                "research_bonus_energy"
            ] * status.alloc_research_energy * (
                100 * cmdTickProduction_research
                + 1.2 * status.current_research_funding
                + artiBonus
            ) / 10000.0 * specopEnlightemntCalc(
                ...
                status.alloc_research_energy * status.population
            ) / (
                600.0 * 100.0
            )
            status.research_points_population += race_info[
                "research_bonus_population"
            ] * status.alloc_research_population * (
                100 * cmdTickProduction_research
                + 1.2 * status.current_research_funding
                + artiBonus
            ) / 10000.0 * specopEnlightemntCalc(
                ...
                status.alloc_research_population * status.population
            ) / (
                600.0 * 100.0
            )
            status.research_points_culture += race_info[
                "research_bonus_culture"
            ] * status.alloc_research_culture * (
                100 * cmdTickProduction_research
                + 1.2 * status.current_research_funding
                + artiBonus
            ) / 10000.0 * specopEnlightemntCalc(
                ...
                status.alloc_research_culture * status.population
            ) / (
                600.0 * 100.0
            )
            status.research_points_operations += race_info[
                "research_bonus_operations"
            ] * status.alloc_research_operations * (
                100 * cmdTickProduction_research
                + 1.2 * status.current_research_funding
                + artiBonus
            ) / 10000.0 * specopEnlightemntCalc(
                ...
                status.alloc_research_operations * status.population
            ) / (
                600.0 * 100.0
            )
            ...
            status.research_points_construction = max(
                0.0, status.research_points_construction
            )
            ...
            status.research_points_population = max(
                0.0, status.research_points_population
            )
            status.research_points_culture = max(0.0, status.research_points_culture)
            status.research_points_operations = max(
                0.0, status.research_points_operations
            )

            # Research funding decay
            status.current_research_funding = max(
                0.0, (0.9 * status.current_research_funding)
            )
            ...
            status.research_percent_military += np.sign(
                race_info.get("research_max_military", 200)
                * (
                    1.0
                    - np.exp(
                        status.research_points_military / (-10.0 * status.networth)
                    )
                )
            )  # np.sign returns a 1 or -1
            status.research_percent_construction += np.sign(
                race_info.get("research_max_construction", 200)
                * (
                    1.0
                    - np.exp(
                        status.research_points_construction / (-10.0 * status.networth)
                    )
                )
            )
            status.research_percent_tech += np.sign(
                race_info.get("research_max_tech", 200)
                * (
                    1.0
                    - np.exp(status.research_points_tech / (-10.0 * status.networth))
                )
            )
            status.research_percent_energy += np.sign(
                race_info.get("research_max_energy", 200)
                * (
                    1.0
                    - np.exp(status.research_points_energy / (-10.0 * status.networth))
                )
            )
            status.research_percent_population += np.sign(
                race_info.get("research_max_population", 200)
                * (
                    1.0
                    - np.exp(
                        status.research_points_population / (-10.0 * status.networth)
                    )
                )
            )
            status.research_percent_culture += np.sign(
                race_info.get("research_max_culture", 200)
                * (
                    1.0
                    - np.exp(status.research_points_culture / (-10.0 * status.networth))
                )
            )
            status.research_percent_operations += np.sign(
                race_info.get("research_max_operations", 200)
                * (
                    1.0
                    - np.exp(
                        status.research_points_operations / (-10.0 * status.networth)
                    )
                )
            )
            ...
            fb = race_info.get("energy_production", 1.0) * (
                1.00 + 0.01 * status.research_percent_energy
            )
            ...
            status.portals_upkeep = max(
                0.0,
                (max(1, status.total_portals) - 1) ** 1.2736
                * 10000.0
                / (1.0 + status.research_percent_culture / 100.0),
            )
            status.population_upkeep_reduction = min(
                status.population_upkeep_reduction,
                (status.buildings_upkeep + status.units_upkeep + status.portals_upkeep),
            )

            # Network Virus Effect.
            for _ in range(opvirus):
                # status.population_upkeep_reduction -= fmax( 0.0, ( status.population_upkeep_reduction * 0.05 ) );
                status.buildings_upkeep *= 1.15

            CMD_ENLIGHT_CRY = (
                0  # its actually a specific value of an enumeration, see enum.h
            )
            CMD_ENLIGHT_MINERAL = 0
            CMD_ENLIGHT_ECTRO = 0
            status.crystal_production = (
                race_info.get("crystal_production", 1.0) * cmdTickProduction_crystal
            ) * specopEnlightemntCalc(status.user.id, CMD_ENLIGHT_CRY)
            ...
            # 	fa /= 2.0;
            # }
            status.crystal_decay = fa * max(
                0.0,
                (status.crystals - (settings_num_value * status.crystal_production)),
            )
            ...
            status.mineral_production = (
                race_info.get("mineral_production", 1.0) * cmdTickProduction_mineral
            ) * specopEnlightemntCalc(status.user.id, CMD_ENLIGHT_MINERAL)
            status.ectrolium_production = (
                race_info.get("ectrolium_production", 1.0) * cmdTickProduction_ectrolium
            ) * specopEnlightemntCalc(status.user.id, CMD_ENLIGHT_ECTRO)
            ...
            """
            status.energy_income = (
                status.energy_production
                - status.energy_decay
                - status.buildings_upkeep
                - status.units_upkeep
                + status.population_upkeep_reduction
                - status.portals_upkeep
            )  # - maind.infos[INFOS_ENERGY_TAX];
            status.mineral_income = (
                status.mineral_production
            )  # (status.mineral_production - maind.infos[INFOS_MINERAL_TAX]);
            status.crystal_income = (
                status.crystal_production - status.crystal_decay
            )  # ((status.crystal_production - maind.infos[INFOS_CRYSTAL_TAX] ) - status.crystal_decay);
            status.ectrolium_income = (
                status.ectrolium_production
            )  # (status.ectrolium_production - maind.infos[INFOS_ECTROLIUM_TAX]);

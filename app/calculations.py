...
def explore_FR_cost(num_planets, num_explos_traveling):
    return int(
        np.floor((num_planets + num_explos_traveling) / 4.0 + 3)
    )  # its either +3 or +10 depending on the round settings, not sure which one is normal
    # 27% FR for 97 planets

# TODO Currently contains lots of unspecified variables e.g. target_is_empiremate
...
def calc_overbuild(
    planet_size, total_buildings
):  # include buildings under construction
...
def calc_overbuild_multi(
    planet_size, planet_buildings, new_buildings
):  # planet_buildings just includes existing and under construction
    ob = min(
        max(0, planet_size - planet_buildings), new_buildings
    )  # number of slots left on the planet, or N, whichever one is smaller
    new_buildings -= ob  # remove it from N to find what's left to build
    ob += (
        sum_of_squares(new_buildings + max(planet_buildings, planet_size))
        - sum_of_squares(max(planet_buildings, planet_size))
    ) / (planet_size ** 2)
    return ob
